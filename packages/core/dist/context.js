...
class Context {
    constructor(config = {}) {
        ...
        // Initialize services
        this.embedding = config.embedding || new embedding_1.GeminiEmbedding({
            apiKey: env_manager_1.envManager.get('GEMINI_API_KEY') || 'your-gemini-api-key',
            model: 'gemini-embedding-001',
            ...(env_manager_1.envManager.get('GEMINI_BASE_URL') && { baseURL: env_manager_1.envManager.get('GEMINI_BASE_URL') })
        });
        ...
        for (let i = 0; i < filePaths.length; i++) {
            ...
            try {
                ...
                for (const chunk of chunks) {
                    ...
                    sumChunkSizes += size;
                    if (size <= 500)
                        chunkSizeDistribution['0-500']++;
                    else if (size <= 1000)
                        chunkSizeDistribution['501-1000']++;
                    else if (size <= 2000)
                        chunkSizeDistribution['1001-2000']++;
                    else
                        chunkSizeDistribution['2000+']++;
